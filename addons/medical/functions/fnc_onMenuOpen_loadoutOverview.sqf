...
params ["_display"];
if (GVAR(debug)) then {systemChat format ["[RAA_ACEA] Display %1", _display];};
...
private _ctrlItems = _display displayCtrl IDC_GUI_LOADOUTOVERVIEW_LIST_ITEMS;
private _ctrlWeaps = _display displayCtrl IDC_GUI_LOADOUTOVERVIEW_LIST_WEAPONS;

lbClear _ctrlItems;
lbClear _ctrlWeaps;

// https://community.bistudio.com/wiki/CT_LISTBOX#Scripting_Example
{
	disableSerialization;
//	lbAdd [_ctrl, _x];
